...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="fixed top-4 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none transition-all duration-500">
      <div
        className={`floating-pill-nav flex items-center justify-between gap-3 sm:gap-6 pointer-events-auto w-full transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          scrolled
            ? 'max-w-6xl rounded-2xl px-5 sm:px-8 py-2.5 bg-[#0A192F] text-[#F8F9FA] shadow-2xl shadow-[#0A192F]/30 border border-[#C5A059]/40 backdrop-blur-2xl'
      ...
      >
        ...
        <Link href="/" className="flex items-center gap-2.5 group shrink-0">
          ...
          <span className="font-serif font-bold text-base sm:text-lg text-[#F8F9FA] tracking-tight">
            ClauseGuard
          </span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-1.5 lg:gap-3 text-xs sm:text-sm font-medium">
          ...
          <Link
            ...
            className={`transition-all duration-300 text-xs font-semibold flex items-center gap-1.5 whitespace-nowrap ${
              activeSection === 'overview'
                ? 'bg-[#C5A059]/20 text-[#C5A059] px-3 py-1.5 rounded-full border border-[#C5A059]/40 shadow-[0_0_12px_rgba(197,160,89,0.35)]'
                : 'text-slate-300 hover:text-white px-2.5 py-1.5 rounded-full hover:bg-white/5'
            }`}
          >
            {activeSection === 'overview' && (
          ...
          <span className="inline-flex items-center gap-1.5 text-[11px] font-mono bg-[#112240] text-slate-200 px-2.5 py-1 rounded-full border border-[#C5A059]/20 shadow-inner shrink-0 whitespace-nowrap ml-1">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Link
            href="/upload"
            className="btn-gold rounded-full px-4 sm:px-5 py-2 text-xs font-semibold flex items-center gap-1.5 shadow-lg hover:scale-105 transition-all duration-300 whitespace-nowrap"
          >
